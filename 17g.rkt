...
;; Nehme eine Liste von Atomen und gib eine Liste von Tuplen bestehend
;; aus Atom und der zugehörigen häufigkeit zurück
(check-expect (frequencies '(a c a b b b) empty)
              '((b 3) (c 1) (a 2)))
(check-expect (frequencies '(a a a b b b) empty)
              '((b 3) (a 3)))
(define frequencies (λ [al tl]
                      (cond [(empty? al) tl]
                            [(isInTuple? (first al)
                                         tl)
                             (frequencies
